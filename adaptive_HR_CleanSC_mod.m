function [X, Y, A, mu_store] = adaptive_HR_CleanSC_mod(CSM, plane_distance, freqs, ...
    scan_limits, grid_resolution, mic_poses, c, Num_sources)
    fprintf('\t------------------------------------------\n');
    ...
    for K = 1:N_freqs
        ...
        for I = 1:N_mic
            r_ti = sqrt( (x_t(:,1) - mic_poses(1,I)).^2 + ...
                         (x_t(:,2) - mic_poses(2,I)).^2 + ...
                         (x_t(:,3) - mic_poses(3,I)).^2 );
            h(I, :) = (-1./(4*pi*(r_ti'))).*exp(-1i.*k.*(r_ti'));    
        end
